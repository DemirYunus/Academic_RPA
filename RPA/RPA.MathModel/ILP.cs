...
namespace RPA.MathModel
{
	public class ILP
	{
		...
		public ILP(DataTable dtProcessInstance)
		{			
		...
		{
            #region dtProcess Oluşturma

            // Gerekli kütüphaneler:
            // using System.Data;
            // using System.Linq;

            // 1. Yeni DataTable (dtProcess) oluşturma ve kolonlarını tanımlama
            DataTable dtProcess = new DataTable("dtProcess");

            // Kolon veri tiplerini kendi orijinal verinize göre (string, int, bool vb.) değiştirebilirsiniz.
            dtProcess.Columns.Add("IDProcess", typeof(string));
            dtProcess.Columns.Add("InstanceCount", typeof(int));       // IDProcessInstance sayısı
            dtProcess.Columns.Add("Department", typeof(string));
            dtProcess.Columns.Add("Account", typeof(string));
            dtProcess.Columns.Add("Software-1", typeof(string));       // veya typeof(bool) / typeof(int)
            dtProcess.Columns.Add("Software-2", typeof(string));
            dtProcess.Columns.Add("Software-3", typeof(string));

            // 2. dtProcessInstance tablosunu IDProcess kolonuna göre gruplama
            var groupedProcesses = dtProcessInstance.AsEnumerable()
                .GroupBy(row => row.Field<string>("ID_Process"));

            // 3. Her bir grup (IDProcess) için dtProcess tablosuna özet satır ekleme
            foreach (var group in groupedProcesses)
            {
                // Departman, Hesap ve Yazılım bilgileri aynı işlem türü için ortak kabul edildiğinden 
                // gruptaki ilk satırı referans alıyoruz
                DataRow firstRow = group.First();

                // Yeni satır oluşturma
                DataRow newRow = dtProcess.NewRow();

                // Grup anahtarı ve gruptaki eleman sayısını atama
                newRow["IDProcess"] = group.Key;
                newRow["InstanceCount"] = group.Count();

                // Ortak bilgileri ilk satırdan alıp atama
                newRow["Department"] = firstRow["Department"];
                newRow["Account"] = firstRow["Account"];
                newRow["Software-1"] = firstRow["Software-1"];
                newRow["Software-2"] = firstRow["Software-2"];
                newRow["Software-3"] = firstRow["Software-3"];

                // Satırı dtProcess tablosuna ekleme
			...
			#region Kısıt-4: Özel bölüm özel robot 

			for (int m = 0; m < numOfDepartment; m++)
			{
				for (int k = 0; k < numOfProc; k++)//Each
				{
					if (dtProcess.Rows[k][2].ToString() == m.ToString())
					{
						for (int j = 0; j < numOfRobot; j++)//Each
						{
							GRBLinExpr k5 = 0;
							...
							{
								if (dtProcess.Rows[g][2].ToString() != m.ToString())
								{
									k5 += y[g, j];
								}
							}
			...
			#region Kısıt-6: k işlem türünün tüm işlemleri atanmalı
			...
			for (int j = 0; j < numOfRobot; j++)//Each
			{
				...
				{
					GRBLinExpr k7 = 0;
					int numOfInstanceOfThisProc = (int)dtProcess.Rows[k][1];
			...
			#region Kısıt-7: robot yazılım eşleşmesi 
			...
			for (int i = 0; i < numOfProcInstance; i++)//Each
			{
				for (int j = 0; j < numOfRobot; j++)//Each
				{
					...
					{
						model.AddConstr(u[n, j] >= x[i, j] * Convert.ToInt32(dtProcessInstance.Rows[i]["Software-" + (n + 1)]), "k8(" + (i).ToString() + " ProcessInst" + j.ToString() + " Robot" + n.ToString() + " Software)");
					}
				}
			...
			#region Kısıt-8: En erken başlama 
			...
			#region Kısıt-9: En geç tamamlanma 
			...
			#region Kısıt-xx: Gecikme hesabı (İPTAL)
			...
			#region Kısıt-10: j robotu kullanıyorsa 1 			
			...
			#region Kısıt-11: s[i,j] robot kapasitesi/planlama ufku 
			...
			#region Kısıt-12: Doğrusallaştırma-1 
			...
			#region Kısıt-13: Doğrusallaştırma-2
			...
			#region Kısıt-14: Doğrusallaştırma-3
			...
			#region Kısıt-15: Ya i-j den önce ya da j-i den önce (Robot için))
			...
			#region Kısıt-16: s[i,j] =s[i,l] robot hesap eşleşmesi 

			for (int l = 1; l < numOfAccount; l++)//Each l=0 hesap gerektirmez !
			{
				for (int i = 0; i < numOfProcInstance; i++)//Each
				{
					if (dtProcessInstance.Rows[i]["Account"].ToString() == l.ToString())
					{
						GRBLinExpr k18 = 0;
						for (int j = 0; j < numOfRobot; j++)//Sum
						{
			...
			#region Kısıt-17: Aynı hesap içinde sıralama 
			for (int l = 1; l < numOfAccount; l++)//Each l=0 hesap gerektirmez !
			{
				for (int i = 0; i < numOfProcInstance; i++)//Each
				{
					if (dtProcessInstance.Rows[i]["Account"].ToString() == l.ToString())
					{
						for (int f = 0; f < numOfProcInstance; f++)//Each
						{
							if (dtProcessInstance.Rows[f]["Account"].ToString() == l.ToString())
							{
								if (i != f)
								{
									int processingTime = Convert.ToInt32(dtProcessInstance.Rows[i][7]);
			...
			#region Kısıt-18: Ya i-j den önce ya da j-i den önce (Hesap için)

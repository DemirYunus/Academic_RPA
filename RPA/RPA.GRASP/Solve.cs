    ...
    public class Solve
    {
        public static List<Robot> SolveGRASP(List<TaskProcess> lstProcess)
        {
            ...
            do
            {
                ...
                TaskProcess candidateTaskProcess = slct.SelectByProcessingTime_ValueBased(lstOrderedTaskProses, 0.3);
                ...
                if (candidateTaskProcess.Department != 0)//Departman bilgisi var ise
                {
                    ...
                    candidateRobots = robotListGenerator.SortByAllocatedDepartment(RobotSortRule.IdleWindowAverageDurationAscending);
                }
                else
                {
                ...
                // Seçim metodunu çağır (Gelecekte SelectFirstFit yerine SelectBestFit yazarak stratejiyi anında değiştirebilirsiniz)
                //Burada uygun robot ve ilgili TaskProses teki instance lar için uygun boşluklar elde ediliyor. 
                //  Robot seçimi yukarıda yapıldı (RobotListGenerator). Şimdi seçilen robot için uygun boşluklar kontrol ediliyor.
                //  Robotun idle window bilgisi ve diğer instance ların window ve Account bilgisi dikkate alınarak her instance için uygun boşluklar belirleniyor.    
                RobotSelectionResult robotTimeWindowResult = RobotSelection.SelectFirstFit(candidateTaskProcess, candidateRobots, lstOrderedTaskProses);
                ...
                {
                    // Aday sonuç içindeki çoklu pencerelerden BEST FIT kuralına göre tek bir pencere seç

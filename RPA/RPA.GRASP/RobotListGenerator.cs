...
public class RobotListGenerator
{
    ...
    {
        return _rawRobotList.OrderBy(r => r.IIR?.Count ?? 0).ToList();
    }

    public List<Robot> SortByIdleWindowCountAscending()
    {
        return _rawRobotList.OrderBy(r => r.LstIdleWindow?.Count ?? 0).ToList();
    }

    public List<Robot> SortByIdleWindowAverageDurationAscending()
    {
        return _rawRobotList.OrderBy(r => GetAverageIdleWindowDuration(r)).ToList();
    }

    public List<Robot> SortByIirCountDescending()
    {
        return _rawRobotList.OrderByDescending(r => r.IIR?.Count ?? 0).ToList();
    }

    public List<Robot> SortByIdleWindowCountDescending()
    {
        return _rawRobotList.OrderByDescending(r => r.LstIdleWindow?.Count ?? 0).ToList();
    }

    public List<Robot> SortByIdleWindowAverageDurationDescending()
    {
        return _rawRobotList.OrderByDescending(r => GetAverageIdleWindowDuration(r)).ToList();
    }
    ...
    {
        var ordered = _rawRobotList.OrderByDescending(r => CountMatchingSoftware(r));
